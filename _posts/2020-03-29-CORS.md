...
>
> 白話: CORS是一個(防禦)機制，允許**瀏覽器**向不同網域的server取用資源(ex: img, style, script, iframe, video)。CORS是一個瀏覽器技術規範，判斷要阻擋或是允許不同網域的資源存取。

![browser and node.js flow](https://imgur.com/zJs2GhO.jpg)



**基於安全性的考量，瀏覽器預設會限制網頁做跨網域的連線**。如果是透過node.js送出請求，就不會受到限制!
...
由Server端做調整設定，於Response Header加上`Access-Control-Allow-Origin`
